...
public class CommandKit implements CommandExecutor {
	...
	public void setLevel(Player player, int level) {
		...
		if (player.hasPermission(PermissionType.GUNGAME_SKIP_LEVEL_10.getPermissionToString()) && level % 10 == 0 && level != 0) {
			level++;
		} else if (player.hasPermission(PermissionType.GUNGAME_SKIP_LEVEL_15.getPermissionToString()) && (level % 15 == 0 && level != 0)) {
			level++;
		} else if (player.hasPermission(PermissionType.GUNGAME_SKIP_LEVEL_20.getPermissionToString()) && (level % 20 == 0 && level != 0)) {
			level++;
		} else if (player.hasPermission(PermissionType.GUNGAME_SKIP_LEVEL_25.getPermissionToString()) && (level % 25 == 0 && level != 00)) {
			level++;
		}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String arg2, String[] args) {
		...
		if (player.isOp()) {
			if (args.length == 0) {
				...
				String kits = "";
				for (String kit : this.kits_content.keySet())
					kits += kit + ",";
